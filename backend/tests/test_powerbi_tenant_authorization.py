...
def make_client() -> TestClient:
    ...
    app.dependency_overrides[analytics.get_db] = get_test_db
...
def dataset_ids(response):
    assert response.status_code == 200
    return sorted(item["inspection_id"] for item in response.json())


def test_unauthenticated_powerbi_returns_401():
    client = make_client()
    seed_dataset()

    response = client.get("/analytics/powerbi")

    assert response.status_code == 401


def test_admin_can_access_all_powerbi_rows():
    client = make_client()
    seed_dataset()

    response = client.get("/analytics/powerbi", headers=auth("dev-token"))

    assert dataset_ids(response) == [1, 2]


def test_powerbi_excludes_unauthorized_tenant_inspection_rows():
    client = make_client()
    seed_dataset(membership_tenant="tenant-a")

    response = client.get("/analytics/powerbi", headers=auth("viewer-token"))

    assert dataset_ids(response) == [1]


def test_disabled_membership_sees_no_powerbi_rows():
    client = make_client()
    seed_dataset(membership_tenant="tenant-a", membership_enabled=False)

    response = client.get("/analytics/powerbi", headers=auth("viewer-token"))

    assert dataset_ids(response) == []


def test_no_membership_sees_no_powerbi_rows():
    client = make_client()
    seed_dataset()
